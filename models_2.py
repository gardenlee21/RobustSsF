...
class Model(nn.Module):
    def __init__(self, num_cls=4):
        ...
        self.mask_update = nn.Linear(num_cls, num_cls)
    ...
    def forward(self, x, mask):
        ...
        #mask_full = bool2int(full_modality)
        #init_mask = bool2int(init_mask)
        #print(mask_full, init_mask)
        #mask = self.mask_update(init_mask.float()) #learnable parameter
        #print(mask)
        mask_couple = mask_full ^ mask
        #print(mask, mask_couple)
        fuse_pred_1, prm_preds_1 = self.decoder_fuse(x1, x2, x3, x4, mask)
        fuse_pred_2, prm_preds_2 = self.decoder_fuse(x1, x2, x3, x4, mask_couple)

        fuse_pred_full, prm_preds_full = self.decoder_fuse(x1, x2, x3, x4, mask_full)
        ...
        if self.is_training:
            ...
            return (fuse_pred_1, fuse_pred_2, fuse_pred_full)#, (flair_pred, t1ce_pred, t1_pred, t2_pred), (prm_preds_1, prm_preds_2, prm_preds_full)
        return (fuse_pred_1, fuse_pred_2, fuse_pred_full)

def bool2int(mask_bool):
    mask_int = []
    mask_bool = torch.squeeze(mask_bool, 0)
    for eachmask in mask_bool:
        mask_int.append(int(eachmask==True))
    return torch.from_numpy(np.array(mask_int)).cuda()